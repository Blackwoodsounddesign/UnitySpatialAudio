...
public class SpatialAudioObject : MonoBehaviour
{
    ...
    [Range(0,5), SerializeField, Tooltip("0 is almost no spatial effect, 1 is max effect.")]
    ...
    public SpatialAudioObject()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            delayLines[i] = new DelayLine(24000); 
    ...
    {
        if (volumeRollOff == VolumeRollOff.Logorithmic)
        {
            if (direction.GetDistance() > minimumDistance)
            {
                m_distance = direction.GetDistance() - minimumDistance + 1;
        ...
        else if(volumeRollOff == VolumeRollOff.Inverse)
        {
            if (direction.GetDistance() > minimumDistance)
            {
                m_distance = direction.GetDistance() - minimumDistance + 1;
        ...
        else if(volumeRollOff == VolumeRollOff.Linear)
        {
            if (direction.GetDistance() > minimumDistance)
            {
                m_distance = direction.GetDistance() - minimumDistance + 1;
        ...
        if (direction.GetDistance() < minimumDistance && m_distance != 1)
        {
            ...
            //makes sure to set this back to zero when inside of the minimum distance radius
            if (m_distance < 1)
                m_distance = Mathf.Clamp(m_distance + 0.05f, 1f, 1.6f);
    ...
    void SetSpatialParameters()
    {
        ...
        if (m_azimuth > 0)
        {
            ...
            spatialFilters[0].SetFilterParameters(sample_rate, 290f, -5f * ITD_delayLeft);
        ...
        else
        {
            ...
            spatialFilters[1].SetFilterParameters(sample_rate, 290f, -5f * ITD_delayRight);
